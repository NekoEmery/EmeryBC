...
export class EBCDrawer {
    ...
    private saveExprPanelPosition(pos: { x: number; y: number } | null): void {
        try {
            if (!Player.ExtensionSettings.EmeryBC) Player.ExtensionSettings.EmeryBC = {};
            (Player.ExtensionSettings.EmeryBC as Record<string, unknown>).exprPanelPos = pos ?? null;
            ServerPlayerExtensionSettingsSync("EmeryBC");
        } catch { /* ignore */ }
    }

    private loadExprPanelPosition(): { x: number; y: number } | null {
        try {
            const store = Player.ExtensionSettings.EmeryBC as Record<string, unknown> | undefined;
            const v = store?.exprPanelPos as { x?: unknown; y?: unknown } | null | undefined;
            if (v && typeof v.x === "number" && typeof v.y === "number") return { x: v.x, y: v.y };
            return null;
        } catch { return null; }
    }
    ...
    private renderButtons(): void {
        ...
        antiLbl.textContent = "Restraint Protection";
    ...
    private renderPoses(): void {
        ...
        for (const group of EXPR_GROUPS) {
            ...
            const isWrap = group === "Emoticon";

            const row = document.createElement("div");
            row.style.cssText = isWrap
                ? "display:flex;flex-direction:column;gap:2px;min-width:0;"
                : "display:flex;align-items:center;gap:3px;min-width:0;";

            const label = document.createElement("span");
            label.style.cssText = "font-family:'Trebuchet MS',serif;font-size:9px;color:#7a5a6a;" +
                (isWrap ? "" : "width:52px;flex-shrink:0;") +
                "text-transform:uppercase;letter-spacing:0.03em;";
            label.textContent = EXPR_GROUP_LABELS[group] ?? group;
            row.appendChild(label);

            const scroll = document.createElement("div");
            scroll.style.cssText = isWrap
                ? "display:flex;flex-wrap:wrap;gap:2px;padding-bottom:2px;"
                : "display:flex;gap:2px;overflow-x:auto;flex:1;scrollbar-width:none;padding-bottom:2px;";
            ...
            noneBtn.addEventListener("click", () => {
                setActive(null);
                try { applyExprGroup(group, null); } catch { /* ignore */ }
            });
            ...
            for (const name of options) {
                ...
                btn.addEventListener("click", () => {
                    setActive(name);
                    try { applyExprGroup(group, name); } catch { /* ignore */ }
                });
        ...
        const renderPresetList = (): void => {
            while (presetListEl.firstChild) presetListEl.removeChild(presetListEl.firstChild);
