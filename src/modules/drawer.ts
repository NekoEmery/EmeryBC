    ...
    private renderDev(): void {
        ...
        if (Player.MemberNumber && CREDITED_IDS.has(Player.MemberNumber)) {
            makeSection("Stat Editor", "EBC_statEditorCollapsed", true, (cnt) => {
                ...
                const getRepArr = (): Array<{ Type: string; Value: number }> => {
                    const r = (Player as unknown as Record<string, unknown>).Reputation;
                    return Array.isArray(r) ? (r as Array<{ Type: string; Value: number }>) : [];
                };
                ...
                const rebuildRepSection = (): void => {
                    ...
                    if (repArr.length === 0) {
                        ...
                        for (const rep of repArr) {
                            ...
                            const inp = document.createElement("input");
                            inp.type = "number"; inp.value = String(rep.Value);
                            inp.style.cssText = `${FONT}font-size:10px;width:62px;background:#1a0810;color:#f0d8ec;border:1px solid #4c2537;border-radius:3px;padding:2px 5px;text-align:right;outline:none;`;
                    ...
                    while (repAddSel.firstChild) repAddSel.removeChild(repAddSel.firstChild);
                ...
                repAddInp.type = "number"; repAddInp.value = "0";
                ...
                repAddBtn.addEventListener("click", () => {
                    ...
                    if (!repArr.find(r => r.Type === repType)) {
                        ...
                        if (upd?.QueueData) upd.QueueData({ Reputation: repArr });
                    }
                    rebuildRepSection();
                });
