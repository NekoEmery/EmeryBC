...
function normalizeForSearch(s: string): string {
    if (!s) return s;
    try { s = s.normalize("NFKD"); } catch { /* ignore */ }
    ...
    const MATH_STYLES: [number, number][] = [
    ...
    return [...s].map(ch => {
        ...
        if (cp < 0x1D400 || cp > 0x1D7C3) return ch;
        for (const [cap, low] of MATH_STYLES) {
            if (cp >= cap && cp < cap + 26) return String.fromCharCode(65 + cp - cap); // → A-Z
            if (cp >= low && cp < low + 26) return String.fromCharCode(97 + cp - low); // → a-z
        }
        return ch;
    }).join("");
}
...
export class EBCDrawer {
    ...
    private rebuildEbcTagsStrip(): void {
        ...
        exportTagTA.readOnly = true; exportTagTA.rows = 3;
        ...
        exportTagTA.addEventListener("click", () => {
            exportTagTA.select();
            try { navigator.clipboard.writeText(exportTagTA.value); } catch { /* ignore */ }
        ...
        importTagBtn.addEventListener("click", () => {
            ...
            exportTagPanel.style.display = "none";
            importTagPanel.style.display  = "none";
            if (!isOpen) importTagPanel.style.display = "";
        });
    ...
    private renderScenes(body: HTMLElement): void {
        ...
        // Newer BC versions store Group.Family as an array (e.g. ["Female3DCG","Futanari3DCG"]).
        // Handle both string and array so groups aren't silently excluded.
        const familyMatches = (groupFamily: unknown, target: string): boolean => {
            if (!groupFamily) return true;
            if (Array.isArray(groupFamily)) return (groupFamily as string[]).includes(target);
            return groupFamily === target;
        };

        const getAllGroups = (filter?: "restraint" | "clothes"): GroupEntry[] => {
            try {
                const bcAsset = (window as unknown as Record<string, unknown>).Asset as
                    Array<{ Group: { Name: string; Description?: string; Family?: unknown; IsRestraint?: boolean } }> | undefined;
                ...
                for (const a of bcAsset) {
                    const g = a.Group;
                    if (familyMatches(g.Family, family) && !seen.has(g.Name)) {
        ...
        const getGroupAssets = (groupName: string): AssetEntry[] => {
            try {
                const bcAsset = (window as unknown as Record<string, unknown>).Asset as
                    Array<{ Group: { Name: string; Family?: unknown }; Name: string; Description?: string }> | undefined;
                if (!Array.isArray(bcAsset)) return [];
                const family = (Player as unknown as Record<string, unknown>).AssetFamily as string ?? "Female3DCG";
                const out: AssetEntry[] = [];
                for (const a of bcAsset) {
                    if (a.Group.Name === groupName && familyMatches(a.Group.Family, family)) {
    ...
    private renderNotes(): void {
        ...
        try { chatSettingsCollapsed = localStorage.getItem("EBC_chatSettingsCollapsed") !== "0"; } catch { /* ignore */ }
        ...
        const mkToggleRow = (
            ...
            refresh();
            btn.addEventListener("click", () => { setVal(!getVal()); refresh(); onAfterToggle?.(); });
            row.appendChild(lbl);
            row.appendChild(btn);
            return row;
        };
        ...
        // AFK ON/OFF
        afkBody.appendChild(mkToggleRow(
            t("users.autoReplyWhenAfk"),
            getAfkEnabled,
            (v) => setAfkEnabled(v),
        ));
        ...
        const toggleChatSettings = (): void => {
            ...
            try { localStorage.setItem("EBC_chatSettingsCollapsed", chatSettingsCollapsed ? "1" : "0"); } catch { /* ignore */ }
        };
        chatSettingsChevron.textContent = chatSettingsCollapsed ? "▲" : "▼";
        chatSettingsBody.style.display = chatSettingsCollapsed ? "none" : "flex";
