(function () {
    ...
    function applyColorToGroup(groupName, color) {
        ...
        item.Color = Array.isArray(existing)
            ? existing.map(() => color)
            : color;
        try {
            callBC(() => CharacterRefresh(Player, false));
    ...
    function applyColorZoneToGroup(groupName, zoneIndex, color) {
        var _a;
        const item = InventoryGet(Player, groupName);
        if (!item)
            return false;
        ...
        if (Array.isArray(item.Color)) {
            ...
            colors = [(_a = item.Color) !== null && _a !== void 0 ? _a : "Default"];
        }
        if (zoneIndex < 0 || zoneIndex >= colors.length)
            return false;
    ...
    function applyColorsToGroup(groupName, colors) {
        var _a, _b, _c;
        const item = InventoryGet(Player, groupName);
        if (!item)
            return false;
        if (Array.isArray(item.Color)) {
            const zoneCount = item.Color.length;
            const applied = [];
            for (let i = 0; i < zoneCount; i++) {
                applied.push((_b = (_a = colors[i]) !== null && _a !== void 0 ? _a : colors[colors.length - 1]) !== null && _b !== void 0 ? _b : "Default");
            }
            item.Color = applied;
        }
        else {
    ...
    function getGroupColors(groupName) {
        ...
        if (Array.isArray(item.Color))
            return [...item.Color];
        return [(_a = item.Color) !== null && _a !== void 0 ? _a : "Default"];
    }
    // Return zone names for a worn item by reading Asset.Layer[].Name.
    function getGroupZoneNames(groupName) {
        var _a;
        const item = InventoryGet(Player, groupName);
        if (!item)
            return [];
        const colors = Array.isArray(item.Color) ? item.Color : [(_a = item.Color) !== null && _a !== void 0 ? _a : "Default"];
        const assetRaw = item.Asset;
        const layers = Array.isArray(assetRaw.Layer) ? assetRaw.Layer : [];
        return colors.map((_, i) => {
            var _a;
            const layer = layers[i];
            if (!layer)
                return `Zone ${i + 1}`;
            const name = (_a = layer.Name) === null || _a === void 0 ? void 0 : _a.trim();
    ...
    function deleteRestraintPreset(id) {
        saveRestraintPresets(getRestraintPresets().filter(p => p.id !== id));
    }
    function renameRestraintPreset(id, name) {
    ...
    function getRoomMembers() {
        ...
        try {
            ...
            return room
                ...
                .map(c => ({
                id: c.MemberNumber,
                name: c.Nickname || c.Name || String(c.MemberNumber),
            }));
        }
        catch (_b) {
            return [];
    ...
    function getRoomMemberItems(memberId) {
        var _a;
        try {
            const room = (_a = window.ChatRoomCharacter) !== null && _a !== void 0 ? _a : [];
...
`;
    ...
    function pointerCoords(e) {
        ...
        if (typeof TouchEvent !== "undefined" && e instanceof TouchEvent) {
            const t = (_a = e.touches[0]) !== null && _a !== void 0 ? _a : e.changedTouches[0];
            return { clientX: (_b = t === null || t === void 0 ? void 0 : t.clientX) !== null && _b !== void 0 ? _b : 0, clientY: (_c = t === null || t === void 0 ? void 0 : t.clientY) !== null && _c !== void 0 ? _c : 0 };
        }
    ...
    function addPointerDown(el, onDown) {
    ...
    class EBCDrawer {
        ...
        renderOutfits() {
            ...
            const renderTagMgmt = () => {
                ...
                newTagInp.addEventListener("keydown", (e) => { if (e.key === "Enter")
                    addTagBtn.click(); });
        ...
        buildColorPickerWidget(initialHex, onChange) {
            ...
            const rgbToHex = (r, g, b) => "#" + [r, g, b].map(x => clamp(x, 0, 255).toString(16).padStart(2, "0")).join("");
            const hexToRgb = (hex) => {
            ...
            let svDrag = false;
            const svMM = (e) => { if (svDrag)
                moveSv(e.clientX, e.clientY); };
            const svMU = () => { svDrag = false; };
            const svTM = (e) => { if (svDrag && e.touches[0]) {
                e.preventDefault();
            ...
            svBox.addEventListener("touchstart", (e) => { svDrag = true; if (e.touches[0]) {
                e.preventDefault();
                moveSv(e.touches[0].clientX, e.touches[0].clientY);
            } }, { passive: false });
            // Drag — hue bar
            ...
            let hueDrag = false;
            const hueMM = (e) => { if (hueDrag)
                moveHue(e.clientX); };
            const hueMU = () => { hueDrag = false; };
            const hueTM = (e) => { if (hueDrag && e.touches[0]) {
                e.preventDefault();
            ...
            hueBar.addEventListener("touchstart", (e) => { hueDrag = true; if (e.touches[0]) {
                e.preventDefault();
                moveHue(e.touches[0].clientX);
            } }, { passive: false });
        ...
        renderPalettes(body) {
            ...
            const build = () => {
                ...
                pickerCleanup === null || pickerCleanup === void 0 ? void 0 : pickerCleanup();
                pickerCleanup = null;
                setPickerHex = null;
                while (container.firstChild)
                    container.removeChild(container.firstChild);
                ...
                clrBtn.addEventListener("click", () => {
                    selectedColor = null;
                    setPickerHex === null || setPickerHex === void 0 ? void 0 : setPickerHex("#cf6f98");
                    updateSelBar();
                    updateSwatchGrid();
                    updateLabel();
                });
                ...
                const pickerWidget = this.buildColorPickerWidget(selectedColor !== null && selectedColor !== void 0 ? selectedColor : "#cf6f98", (hex) => {
                    selectedColor = hex;
                    updateSelBar();
                    updateSwatchGrid();
                });
                const w = pickerWidget;
                pickerCleanup = w._cleanup;
                setPickerHex = w._setValue;
                ...
                updateSwatchGrid = () => {
                    ...
                    for (const c of saved) {
                        ...
                        sw.addEventListener("click", () => {
                            selectedColor = c;
                            setPickerHex === null || setPickerHex === void 0 ? void 0 : setPickerHex(c);
                            updateSelBar();
                            updateSwatchGrid();
                        });
                ...
                try {
                    for (const item of Player.Appearance) {
                        ...
                        if (RESTRAINT_GROUPS.has(g.Name)) {
                            const itemDesc = ((_a = item.Asset.Description) === null || _a === void 0 ? void 0 : _a.trim()) || item.Asset.Name;
                            const groupDesc = ((_b = g.Description) === null || _b === void 0 ? void 0 : _b.trim()) || g.Name;
                    ...
                    for (const w of worn) {
                        ...
                        const rebuildZones = () => {
                            ...
                            while (zonesPanel.firstChild)
                                zonesPanel.removeChild(zonesPanel.firstChild);
                            ...
                            saveBtn.addEventListener("click", () => {
                                ...
                                if (!name) {
                                    saveInp.style.borderColor = "#cf6f98";
                                    return;
                                }
                ...
                const renderPal = () => {
                    while (palContainer.firstChild)
                        palContainer.removeChild(palContainer.firstChild);
                    if (palCollapsed)
                        return;
                    const buildPRow = (p, rerender) => {
                        const prow = document.createElement("div");
                        prow.className = "ebc-palette-row";
                        const sw = document.createElement("div");
                        sw.className = "ebc-palette-swatch";
                        const cols = [].concat(...Object.values(p.colorMap).map(c => Array.isArray(c) ? c : [c])).filter(Boolean).slice(0, 8);
                        for (const c of cols) {
                            const d = document.createElement("div");
                            d.className = "ebc-palette-dot";
                            d.style.background = c;
                            sw.appendChild(d);
                        }
                        const ni = document.createElement("input");
                        ni.className = "ebc-palette-name";
                        ni.value = p.name;
                        ni.maxLength = 30;
                        ni.title = "Rename";
                        ...
                        db.addEventListener("click", () => {
                            if (!dp) {
                                ...
                                db.textContent = "Sure?";
                                dt = window.setTimeout(() => { dp = false; db.classList.remove("confirm"); db.textContent = "×"; }, 2500);
                            }
                            else {
                                if (dt)
                    ...
                    const buildSRow = (ph, lbl, onSave, rerender) => {
                        const w = document.createElement("div");
                        w.style.cssText = "display:flex;gap:5px;align-items:center;margin-top:4px;";
                        const i = document.createElement("input");
                        i.className = "ebc-form-input";
                        i.style.flex = "1";
                        i.placeholder = ph;
                        i.maxLength = 30;
                        const b = document.createElement("button");
                        b.className = "ebc-wear-btn";
                        b.textContent = lbl;
                    ...
                    for (const p of ops)
                        palContainer.appendChild(buildPRow(p, renderPal));
                    if (!ops.length) {
                    ...
                    palContainer.appendChild(pd);
                    const rLbl = document.createElement("div");
                    rLbl.className = "ebc-import-hint";
                    rLbl.style.cssText = "font-weight:600;margin-bottom:3px;";
                    rLbl.textContent = "RESTRAINTS ⛓";
                    palContainer.appendChild(rLbl);
        ...
        buildOutfitRow(o, body) {
            ...
            const renderTagChips = () => {
                ...
                const outfitTagIds = new Set((_a = o.tagIds) !== null && _a !== void 0 ? _a : []);
                for (const tag of allTags) {
                    if (!outfitTagIds.has(tag.id))
                        continue;
            ...
            const renderEditTags = () => {
                ...
                for (const tag of allTags) {
                    ...
                    btn.addEventListener("click", () => {
                        var _a;
                        const ids = new Set((_a = o.tagIds) !== null && _a !== void 0 ? _a : []);
                        if (ids.has(tag.id))
                            ids.delete(tag.id);
                        else
                            ids.add(tag.id);
                ...
                const renderFields = (type) => {
                    ...
                    if (type === "pose") {
                        ...
                        const updateStateRow = () => {
                            ...
                            if (!info.types.includes(equipPropertyType))
                                equipPropertyType = "";
                        ...
                        captureBtn.addEventListener("click", () => {
                            ...
                            try {
                                ...
                                if (typeof (prop === null || prop === void 0 ? void 0 : prop.HeightModifier) === "number") {
                                    equipHeightModifier = prop.HeightModifier;
                                    heightInp.value = String(equipHeightModifier);
                                }
                            }
                            catch ( /* ignore */_d) { /* ignore */ }
                ...
                const getStep = () => {
                    ...
                    switch (currentType) {
                        ...
                        case "equip-clothes":
                            ...
                            if (equipHeightModifier !== undefined)
                                step.heightModifier = equipHeightModifier;
                            break;
                        case "unequip":
        ...
        renderFriendRows(body) {
            ...
            if (friendList.length > 0) {
                ...
                const buildFriendRow = (num, container) => {
                    var _a;
                ...
                if (offlineFriends.length > 0) {
                    ...
                    offlineToggle.addEventListener("click", () => {
                        ...
                        if (!this.offlineFriendsCollapsed && !offlineContainer.firstChild) {
                            for (const num of offlineFriends)
                                buildFriendRow(num, offlineContainer);
                        }
                    });
            ...
            if (Player.MemberNumber && CREDITED_IDS.has(Player.MemberNumber)) {
                makeSection("Stat Editor", "EBC_statEditorCollapsed", true, (cnt) => {
                    ...
                    const rebuildRepSection = () => {
                        ...
                        const repRaw = Player.Reputation;
                        const repArr = Array.isArray(repRaw) ? repRaw : [];
                    ...
                    cnt.appendChild(subLbl("Account"));
                    {
                        ...
                        row.appendChild(lbl);
                        row.appendChild(moneyInp);
                        row.appendChild(addMoneyBtn);
                        row.appendChild(setMoneyBtn);
                        cnt.appendChild(row);
                    }
                    // ── Unlock all items ───────────────────────────────────────────
                    cnt.appendChild(subLbl("Actions"));
                    {
                        ...
                        unlockBtn.addEventListener("click", () => {
                            ...
                            try {
                                ...
                                const assetList = (_a = window.Asset) !== null && _a !== void 0 ? _a : [];
                                if (!invAdd) {
                                    unlockBtn.textContent = "InventoryAdd not found";
                                    return;
                                }
                                for (const asset of assetList) {
                                    try {
                                        invAdd(Player, asset.Name, asset.Group.Name, false);
                                    }
                                    catch ( /* skip */_b) { /* skip */ }
                                }
                    ...
                    applyBtn.addEventListener("click", () => {
                        ...
                        try {
                            const repRaw = Player.Reputation;
                            const rep = Array.isArray(repRaw) ? repRaw : [];
        ...
        renderPuppy() {
            ...
            const getBarks = () => {
                ...
                if (Array.isArray(store.barks))
                    return store.barks;
                const legacy = Array.isArray(store.customBarks) ? store.customBarks : [];
            ...
            const rebuildBarkList = () => {
                while (barkList.firstChild)
                    barkList.removeChild(barkList.firstChild);
