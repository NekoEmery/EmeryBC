...
`;
    ...
    class EBCDrawer {
        ...
        setup() {
            ...
            // Word input helper
            const makeSwWordRow = (label, getter, setter) => {
                ...
                inp.addEventListener("blur", () => { inp.style.borderColor = "#3a1928"; if (inp.value.trim())
                    setter(inp.value.trim()); inp.value = getter(); });
                row.appendChild(lbl);
                row.appendChild(inp);
                swInner.appendChild(row);
            };
            try {
                makeSwWordRow("Yellow:", () => getSafewordConfig().yellowWord, (v) => setSafewordConfig(Object.assign(Object.assign({}, getSafewordConfig()), { yellowWord: v })));
                makeSwWordRow("Red:", () => getSafewordConfig().redWord, (v) => setSafewordConfig(Object.assign(Object.assign({}, getSafewordConfig()), { redWord: v })));
            }
            catch ( /* ignore */_c) { /* ignore */ }
            // Grace duration row
            ...
            swGraceDurInp.type = "number";
            swGraceDurInp.min = "0";
            swGraceDurInp.max = "9999";
            ...
            try {
                swGraceDurInp.value = String(Math.round(getSafewordConfig().graceDurationMs / 60000));
            }
            catch (_d) {
                swGraceDurInp.value = "5";
            }
            ...
            // Outfit selectors
            const makeSwOutfitRow = (label, labelColor, getOutfitId, setOutfitId) => {
                var _a;
                const row = document.createElement("div");
                row.style.cssText = "display:flex;align-items:center;gap:6px;";
                const lbl = document.createElement("span");
            ...
            makeSwOutfitRow("Y outfit:", "#c8b840", () => getSafewordConfig().yellowOutfitId, (id) => setSafewordConfig(Object.assign(Object.assign({}, getSafewordConfig()), { yellowOutfitId: id })));
            makeSwOutfitRow("R outfit:", "#e06060", () => getSafewordConfig().redOutfitId, (id) => setSafewordConfig(Object.assign(Object.assign({}, getSafewordConfig()), { redOutfitId: id })));
            // Hint
